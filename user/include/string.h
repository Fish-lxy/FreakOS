...
// The len is the max-length that function will scan
static inline strnlen(const char *s, uint32_t len) {
    uint32_t cnt = 0;
    while (cnt < len && *s++ != '\0') {
        cnt++;
    }
...
static inline void bzero(void *dest, uint32_t len) { memset(dest, 0, len); }
