...
static showFAT(FATFS* fs);
static FRESULT ls(const char* path);

int initFAT() {
    printk("\n");

    FatFs[0] = kmalloc(sizeof(FATFS) * _DRIVES);
    DIR dj;
    char* d = "0:/";
    int i = auto_mount(&d, &dj.fs, 0);
    // printk("mount:%d\n", i);
...
FRESULT auto_mount(	/* FR_OK(0): successful, !=0: any error occured */
    ...
    BYTE drv, fmt, * tbl;
    DSTATUS stat;
    DWORD bootsect = BOOTSECT, fatsize, totalsect, maxclust;
    const char* p = *path;
    FATFS* fs;
...
FRESULT f_mount(
    ...
    if (FatFs[drv])
        FatFs[drv]->fs_type = 0;	/* Clear old object */
...
FRESULT f_opendir(
    DIR* dj,			/* [OUT] Pointer to directory object to create */
...
FRESULT f_readdir(
    DIR* dj,			/* [IN] Pointer to the directory object */
