...
static void lockBlockCache() { accquireSem(&BlockCacheSem); }
...
// 直接磁盘IO，一次仅可请求一个磁盘块
static int32_t disk_io_direct_oneblock(Device_t *blockdev, void *buffer,
...
// 经过块缓冲区的磁盘IO，LRU算法管理
static int bcache_rw(Device_t *blockdev, void *buffer, uint32_t secno,
                     bool write) {
    lockBlockCache();
...
// 直接读写磁盘
static int32_t disk_io_direct(Device_t *blockdev, IOrequest_t *req) {
    if (blockdev == NULL) {
        panic("blockdev is NULL in disk_io!");
    }
    if (blockdev->type != BlockDevice) {
...
// 分多次直接读写磁盘
static int32_t disk_io_block(Device_t *blockdev, void *buffer, uint32_t secno,
...
void testBCache() {
    ...
    Device_t *dev = getDeviceWithId(0);
    IOrequest_t req;
