...
extern BlockDevice_t *getDeviceWithId(uint32_t devid); // dev.c

// 读磁盘，分块多次读取
static DRESULT _disk_read(BlockDevice_t *blockdev, void *buffer, uint32_t secno,
                           uint32_t nsecs) {
    IOrequest_t req;
    req.io_type = IO_READ;
    req.nsecs = 1;
...
static DRESULT _disk_write(BlockDevice_t *blockdev, void *buffer, uint32_t secno,
                           uint32_t nsecs) {
    IOrequest_t req;
...
DRESULT fatbase_disk_read(BYTE devid, void *buffer, uint32_t secno,
                          uint32_t nsecs) {
    BlockDevice_t *dev = getDeviceWithId(devid);
